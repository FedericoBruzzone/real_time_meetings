...
namespace DSP
{
    namespace IIR
    {
                                                                                 ...
                                                                                 NumericType gainFactor)
        {
            ...
            auto aminus1 = A - 1;
            auto aplus1 = A + 1;
            auto omega = (2 * MathConstants<NumericType>::pi * std::max (cutOffFrequency, static_cast<NumericType> (2.0))) / static_cast<NumericType> (sampleRate);
                                                                                   ...
                                                                                   NumericType gainFactor)
        {
            ...
            auto A = std::max (static_cast<NumericType> (0.0), std::sqrt (gainFactor));
            auto omega = (2 * MathConstants<NumericType>::pi * std::max (frequency, static_cast<NumericType> (2.0))) / static_cast<NumericType> (sampleRate);
        ...
        double Coefficients<NumericType>::getMagnitudeForFrequency (double frequency, double sampleRate) const noexcept
        {
            ...
            std::complex<double> jw = std::exp (-MathConstants<double>::twoPi * frequency * j / sampleRate);
                                                                       ...
                                                                       size_t numSamples, double sampleRate) const noexcept
        {
            ...
            {
                assert (frequencies[i] >= 0 && frequencies[i] <= sampleRate * 0.5);

                std::complex<double> numerator = 0.0, denominator = 0.0, factor = 1.0;
                std::complex<double> jw = std::exp (-MathConstants<double>::twoPi * frequencies[i] * j / sampleRate);
                                                                   ...
                                                                   size_t numSamples, double sampleRate) const noexcept
        {
            ...
            for (size_t i = 0; i < numSamples; ++i)
            {
                ...
                std::complex<double> numerator = 0.0, denominator = 0.0, factor = 1.0;
                std::complex<double> jw = std::exp (-MathConstants<double>::twoPi * frequencies[i] * j * invSampleRate);
